 ...
 */
SPEC_FUN *spec_lookup( int vnum, char *name )
{
    ...
    sprintf( buf, "Spec_lookup: vnum %%d: %s not found.", name );
    bug( buf, vnum );
    exit( 1 );
    return NULL;
}
...
bool dragon( CHAR_DATA *ch, char *spell_name )
{
    ...
    for ( victim = room_index[ch->in_room].people; victim; victim = v_next )
    {
...
bool spec_executioner( CHAR_DATA *ch )
{
    ...
    char_to_room( create_mobile( real_mobile(MOB_VNUM_CITYGUARD) ),
	ch->in_room );
    char_to_room( create_mobile( real_mobile(MOB_VNUM_CITYGUARD) ),
	ch->in_room );
    return TRUE;
...
bool spec_fido( CHAR_DATA *ch )
{
    ...
    for ( corpse = room_index[ch->in_room].contents; corpse; corpse = c_next )
    {
...
bool spec_janitor( CHAR_DATA *ch )
{
    ...
    for ( trash = room_index[ch->in_room].contents; trash; trash = trash_next )
    {
...
bool spec_thief( CHAR_DATA *ch )
{
	...
	{
	    gold = victim->gold * number_range( 1, 10 ) / 10;
	    ch->gold     += gold / 2;
	    victim->gold -= gold;
	    return TRUE;
	}
