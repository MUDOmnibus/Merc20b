...
const	struct	cmd_index	cmd_table	[] =
{
    /*
    ...
    { "chat",		do_chat,	POS_RESTING,	 0,  LOG_NORMAL	},
    { ".",		do_chat,	POS_RESTING,	 0,  LOG_NORMAL	},
    ...
    { "poofin",		do_poofin,	POS_DEAD,	37,  LOG_NORMAL	},
    { "poofout",	do_poofout,	POS_DEAD,	37,  LOG_NORMAL	},
...
const	struct	social_index	social_table [] =
{
    {
	"accuse",
	"Accuse whom?",
...
void interpret( CHAR_DATA *ch, char *argument )
{
     ...
     * Ugly special case so ' can work.
     */
    strcpy( logline, argument );
    if ( argument[0] == '\'' && isspace(argument[1]) )
	argument[0] = '-';
    ...
    {
	if ( !strncmp( command, cmd_table[cmd].name, length )
	&&   ( !IS_NPC(ch) || cmd_table[cmd].level < MAX_LEVEL - 4 ) )
	{
	    found = TRUE;
...
bool check_social( CHAR_DATA *ch, char *command, char *argument )
{
    ...
    {
	if ( !strncmp( command, social_table[cmd].name, length ) )
	{
	    found = TRUE;
	    break;
	}
...
void do_wizhelp( CHAR_DATA *ch, char *argument )
{
    ...
    send_to_char( "The following immortal comands are available:\n\r", ch );

    buf[0] = '\0';
    for ( cmd = 0, col = 0; cmd_table[cmd].name[0] != '\0'; cmd++ )
    {
	if ( cmd_table[cmd].level < MAX_LEVEL - 4
	||   cmd_table[cmd].level > ch->level )
	    continue;

	sprintf( buf + strlen(buf), "%-11s", cmd_table[cmd].name );
	if ( ++col % 7 == 0 )
	    strcat( buf, "\n\r" );
    }
